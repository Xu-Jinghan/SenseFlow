...
def get_sensor_loader(*args, **kwargs):
    from sensor_loader import get_sensor_loader as _get_sensor_loader

    return _get_sensor_loader(*args, **kwargs)
...
def get_imagenet(batch_size, data_root='/usr/scratch1/datasets/imagenet/', train=True, val=True, sample=False, model=None, **kwargs):
    ...
    if train:
        train_path = os.path.join(data_root, 'val')
        print(f'Warning: Using val set as train set from {train_path}, as download is too slow.')
