...
from models import vgg, resnet, cimae
...
def quantize_model(args, criterion, data_loader, data_loader_test):
    ...
    effective_dataset = args.sensor_source_dataset if args.dataset == 'sensor' else args.dataset
    model = None
    device = torch.device(getattr(args, "device", f"cuda:{args.gpu}" if torch.cuda.is_available() else "cpu"))
    ...
    saved_model = os.path.join(args.model_path, f"{args.model}_{effective_dataset}.pth")

    if effective_dataset == 'imagenet':
    ...
    if model is None:
        raise ValueError(f"Unsupported model/dataset combination: model={args.model}, dataset={effective_dataset}")
    ...
    if effective_dataset != 'imagenet':
        ...
        if not os.path.isfile(saved_model):
            raise FileNotFoundError(
                f"Expected pretrained checkpoint at {saved_model}, but it does not exist. "
                "Refusing to overwrite it with randomly initialized weights."
            )
        state_dict = torch.load(saved_model, map_location=device, weights_only=False)
    ...
    layer_quant = layers if args.dataset == 'sensor' else layers[1:]
    ...
    collect_stats(model, layers, data_loader, device, quant_mode='iw', num_batches=2)
    strict = True
    ...
    with torch.no_grad():
        ...
        for i, (images, labels) in tqdm(enumerate(data_loader), total=num_batches):
            ...
            if ((i + 1) % print_freq == 0) or (i + 1 == num_batches):
                ...
                args.logger(
                    f"Eval{log_suffix}: [{i + 1}/{num_batches}] "
                    f"loss={avg_loss:.4f} top1={top1:.2f}% elapsed={elapsed:.2f}s"
                )

        avg_loss = total_loss / max(total, 1)
        top1 = 100.0 * correct / max(total, 1)
        args.logger(f"Eval{log_suffix}: completed loss={avg_loss:.4f} top1={top1:.2f}%")
        return top1, avg_loss


def collect_stats(model, layer_quant, data_loader, device, quant_mode='iw',  num_batches=2):
