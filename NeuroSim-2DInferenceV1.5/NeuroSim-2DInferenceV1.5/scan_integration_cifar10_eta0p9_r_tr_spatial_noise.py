...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Scan CIFAR-10 under integration readout for a fixed eta, x2 trap disabled, "
            "R in [0.005, 1], trise=tfall over four decades, spatial variation in {1,2,3,5}%, "
            "and noise in {1e-9, 1e-7}."
        )
    )
    ...
    parser.add_argument("--spatial-values", nargs="+", type=float, default=[1.0, 2.0, 3.0, 5.0])
...
def build_scenarios(args):
    ...
    r_values = [0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0]
    tr_values = [1e-4, 1e-3, 1e-2, 1e-1]
    noise_values = [1e-9, 1e-7]
                ...
                for r_value in r_values:
                    for tr_value in tr_values:
                        scenarios.append(
                            {
                                "scenario_name": (
                                    f"{readout_tag}_fps{int(fps):03d}_sp{int(round(spatial)):02d}_R{r_value:.3f}_eta{args.eta:.2f}_tr{tr_value:.0e}_n{noise_value:.0e}"
                                ).replace(".", "p"),
                                "video_fps": fps,
