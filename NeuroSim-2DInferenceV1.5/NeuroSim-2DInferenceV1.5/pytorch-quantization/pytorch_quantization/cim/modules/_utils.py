...
class QuantMixin():
    ...
    def init_cim(self, cim_args, in_features, out_features):
        ...
        device = torch.device(getattr(cim_args, "device", "cuda" if torch.cuda.is_available() else "cpu"))
        conductance_variation = float(getattr(cim_args, "conductance_variation", 0.0))
        ...
        if cim_args.mem_states_file: # external mean and std
            with open(cim_args.mem_states_file, 'r') as file:
                ...
                if "mean" in header:
                    ...
                    self.mem_mean_gaussian = torch.tensor(means, dtype=torch.float32, device=device)
                    self.mem_std_gaussian = torch.tensor(stds, dtype=torch.float32, device=device)
                    self._cim_args.mem_states = torch.tensor(means, dtype=torch.float32, device=device)
                    if conductance_variation > 0 and torch.count_nonzero(self.mem_std_gaussian) == 0:
                        floor = torch.clamp(torch.abs(self.mem_mean_gaussian).max() * conductance_variation * 1e-3, min=1e-20)
                        self.mem_std_gaussian = torch.clamp(torch.abs(self.mem_mean_gaussian) * conductance_variation, min=floor)
        ...
        # Set intermediate states based on on and off state
            ...
            self._cim_args.mem_states = torch.zeros(num_states, device=device)
            ...
            if conductance_variation > 0:
                self.mem_mean_gaussian = self._cim_args.mem_states.clone()
                floor = torch.clamp(torch.abs(self.mem_mean_gaussian).max() * conductance_variation * 1e-3, min=1e-20)
                self.mem_std_gaussian = torch.clamp(torch.abs(self.mem_mean_gaussian) * conductance_variation, min=floor)
                self.mem_sampling_seed = torch.randint(1, 1000, (self._cim_args.weight_precision,))
        ...
        if (cim_args.output_noise == -1.0):
            with open(cim_args.output_noise_file, 'r') as file:
                ...
                if "output" in header:
                    ...
                    self.output_values = torch.tensor(values, dtype=torch.float32, device=device)
                    if stds:
                        self.output_noise_stds = torch.tensor(stds, dtype=torch.float32, device=device)
                    else:
                        self.output_noise_stds = torch.zeros(0)
                    if means:
                        self.output_noise_means = torch.tensor(means, dtype=torch.float32, device=device)
                    else:
